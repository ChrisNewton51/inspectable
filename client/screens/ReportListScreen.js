...
const ReportListScreen = ({user, editReport, trashReport, changeReportId, searchReports}) => {
    const navigation = useNavigation();
    const dispatch = useDispatch();
    const {reports} = useSelector(state => state.reportList)
    const {search} = useSelector(state => state.reportList)
    ...
    const handleSearch = text => {
        ...
        const filteredData = filter(reports, user => {
            return contains(user, formattedQuery);
        });
        searchReports(filteredData)
        setQuery(text);
    };
                ...
                <View style={styles.headerPopup}>
                    ...
                    <View style={styles.headerPopupRow}>
                        ...
                        <TouchableOpacity 
                            style={styles.headerPopupSection}
                            onPress={() => {
                                trashReport(reportId, currentIndex, user.id, user)
                                setSelect()
                                setRefreshing(true);
                                setOpenModal(false);
                                dispatch(getUserReports(reps))
